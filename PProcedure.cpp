...
Node GetNodeFromEntityID(std::string idType, const std::string &person_id) {
    ...
    if (it == IDMap.end()) {
        ...
        cout<< idType <<"IDMap not found"<<endl;
        return Node(-1);
    }
    auto it2 = myMap.find(it->second);
    if (it2 == myMap.end()) {
        // To Do: Error handling
        cout<< idType <<"Map not found"<<endl;
        return Node(-1);
    }
    return it2->second;
}

Node GetNodeFromGlobalID(std::string idType, const std::string &person_id) {
    auto MapIt = type2Map.find(idType);
    auto& myMap = *MapIt->second;

    auto it2 = myMap.find(person_id);
    if (it2 == myMap.end()) {
        // To Do: Error handling
        cout<< idType <<"Map not found"<<endl;
        return Node(-1);
...
void ic1(const std::vector<GPStore::Value> &args, std::vector<std::vector<GPStore::Value>> &result) {
    ...
    // Node person_node = GetNodeFromEntityID("Person", args[0].toString());
...
void ic2(const std::vector<GPStore::Value> &args, std::vector<std::vector<GPStore::Value>> &result) {
    ...
    // }
    for (unsigned i = 0; i < list_len; ++i) {
        Node friend_node = GetNodeFromGlobalID("Person", friends_list[i]);
        ...
        for (unsigned j = 0; j < messages_len; ++j) {
            Node message_node = GetNodeFromGlobalID("Comment", messages_list[j]);
            long long creation_date = message_node["creationDate"]->toLLong();
            if (creation_date < max_date) {
                std::string content = message_node["content"]->toString();
                std::string message_id = message_node["id"]->toString();
